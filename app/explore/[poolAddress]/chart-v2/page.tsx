...
import LightweightChart, { type LightweightChartHandle } from "./LightweightChart";
...
export default function ChartV2Page() {
  ...
  useEffect(() => {
    ...
    async function loadOhlc() {
      ...
      try {
        const tfMap: Record<string, { path: string; agg: string }> = {
        ...
        const url = `https://api.geckoterminal.com/api/v2/networks/base/pools/${poolAddress}/ohlcv/${selected.path}?aggregate=${selected.agg}&limit=500`;
        ...
        if (pair?.baseToken?.symbol) {
          ...
          const daysMap: Record<string, number> = { "1m": 1, "5m": 1, "15m": 1, "1h": 1, "4h": 1, "1d": 1, "1w": 7 };
  ...
  useEffect(() => {
    ...
    async function fetchPrice() {
      try {
        ...
        const sellToken = "0x4200000000000000000000000000000000000006"; // WETH on Base
        const buyToken = pair.baseToken.address;
        const amountWei = (Number(payAmount) * 1e18).toFixed(0);
        
        console.log("🔍 Fetching 0x price:", { payAmount, sellToken, buyToken, amountWei });
        ...
        // Most tokens use 18 decimals, but we should ideally fetch the actual decimals
        const buyAmount = data?.buyAmount;
        if (!cancelled && buyAmount) {
          // For now, assume 18 decimals (standard for most ERC20 tokens)
          // TODO: Fetch actual token decimals for more accuracy
          const tokenDecimals = 18; // pair?.baseToken?.decimals || 18;
  ...
  }, [payAmount, pair?.baseToken?.address]);
  ...
  const executeSwap = async () => {
    ...
    try {
      ...
      const WETH_ADDRESS = "0x4200000000000000000000000000000000000006"; // Base WETH
      const sellToken = isBuy ? WETH_ADDRESS : pair.baseToken.address;
      const buyToken = isBuy ? pair.baseToken.address : WETH_ADDRESS;
      const sellAmountWei = ethers.parseUnits(payAmount, 18).toString();
      ...
      fetchWethBalance(swapWalletAddress);
      ...
      const tokenSymbol = isBuy ? (pair?.baseToken?.symbol || 'TOKEN') : 'WETH';
      setSwapSuccess({
        type: isBuy ? 'buy' : 'sell',
      ...
      try {
        ...
        if (data.result) {
          ...
          buffer.push({
            ...
            tokenAmount,
            timestamp: Date.now(),
            tokenSymbol: isSell ? (pair?.baseToken?.symbol || "TOKEN") : "ETH",
            decimals,
          });
        }
      } catch (e) {
    ...
    async function fetchTransactions(pairAddress: string) {
      try {
        ...
        const mapped = filtered.map((t: any) => {
          ...
          try {
            if (t.value != null) {
              // const decimals = isErc20 && t.rawContract?.decimal ? parseInt(t.rawContract.decimal, 16) : 18;
              tokenAmount = parseFloat(t.value) || 0;
              tokenAmount = tokenAmount; // already human for external; for erc20 value is amount
            }
          } catch (_) {
            tokenAmount = 0;
          }
      ...
      <div className="flex-1 grid grid-cols-1 xl:grid-cols-[1fr_260px_440px] gap-0 overflow-hidden">
        ...
        <div className="flex flex-col min-h-0 overflow-y-auto">
          ...
          <div className="flex-shrink-0">
            {/* Controls */}
            <div className="px-4 py-2 border-b border-gray-800 flex items-center justify-between bg-gray-950">
          ...
          <div className="px-4 py-3 flex-1 flex flex-col">
            ...
            <div className="bg-gray-900/50 border border-gray-800 p-3">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 text-[11px] text-gray-300">
                  <img src={isBuy ? 'https://assets.coingecko.com/coins/images/2518/small/weth.png' : (pair?.info?.imageUrl || '')} alt="" className="w-4 h-4 border border-gray-700" />
                  <span>{isBuy ? 'WETH' : (pair?.baseToken?.symbol || 'TOKEN')}</span>
                  {walletAddress && (
                    <span className="text-[10px] text-gray-500 ml-1">
                      ({isBuy 
                        ? (walletEthBalance > 0 ? (walletEthBalance < 0.0001 ? walletEthBalance.toFixed(8) : walletEthBalance.toFixed(4)) : '0.0000')
              ...
              {['25%','50%','75%','MAX'].map((v, i) => {
                ...
                return (
                  <button key={v} onClick={()=>{
                    const basis = isBuy ? walletEthBalance : walletTokenBalance;
                    if (basis > 0) {
                      const amount = (basis * pct) / 100;
                      setPayAmount(amount.toFixed(6));
                    }
                  }} disabled={!walletAddress || (isBuy ? walletEthBalance <= 0 : walletTokenBalance <= 0)} className="px-2 py-1 text-[11px] bg-gray-800/60 text-gray-300 hover:text-white hover:bg-blue-500/20 disabled:opacity-50 disabled:cursor-not-allowed">{v}</button>
                );
              })}
            </div>
            ...
            <div className="flex items-center justify-center my-2">
              <button onClick={()=>{ 
                ...
                if (walletAddress) {
                  fetchWethBalance(walletAddress);
                  if (pair?.baseToken?.address) {
                    fetchTokenBalance(walletAddress, pair.baseToken.address);
                  }
                }
            ...
            <div className="bg-gray-900/50 border border-gray-800 p-3 mb-3">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 text-[11px] text-gray-300">
                  <img src={isBuy ? (pair?.info?.imageUrl || '') : 'https://assets.coingecko.com/coins/images/2518/small/weth.png'} alt="" className="w-4 h-4 border border-gray-700" />
                  <span>{isBuy ? (pair?.baseToken?.symbol || 'TOKEN') : 'WETH'}</span>
                </div>
              </div>
              ...
              <>
                ...
                >
                  {isSwapping ? (
                    ...
                    `${isBuy ? 'Buy' : 'Sell'} ${isBuy ? (pair?.baseToken?.symbol || 'TOKEN') : 'WETH'}`
                  )}
                </button>
              </>
            ) : (
